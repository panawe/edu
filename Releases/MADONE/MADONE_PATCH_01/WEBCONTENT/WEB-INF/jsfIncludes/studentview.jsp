	...
	<tr valign="top">
		<td valign="top"><rich:simpleTogglePanel switchType="ajax"
			...
			<a4j:outputPanel ajaxRendered="true">
				<h:panelGroup id="infoImageHeader3">
					<a4j:mediaOutput id="StudentPhotoHeader2" element="img"
						mimeType="image/gif" style="width:100px; height:100px;"
						createContent="#{studentBean.paint}" value="#{studentBean.random}"
						cacheable="false">
					</a4j:mediaOutput>
				</h:panelGroup>
			</a4j:outputPanel>
		</rich:simpleTogglePanel> <rich:modalPanel id="tabLoading2" autosized="true" width="100"
		...
		</rich:modalPanel> <a4j:status id="status11" forceId="true"
			...
			<rich:tab id="studentDetails" label="#{msg.details}" status="status11">
			...
			<rich:tab id="schooling" label="#{msg.schooling}"
				rendered="#{studentBean.studentSelected }" status="status11">
			...
			<rich:tab status="status11" id="studentMarksAndAverages"
			...
			<rich:tab status="status11" id="correspondence"
				...
				<rich:tabPanel switchType="ajax" width="100%"
					...
					immediate="true">
					<rich:tab status="status11" id="studentCorrespondenceList"
			...
			<rich:tab status="status11" id="enrollment" label="#{msg.enrollment}"
			...
			<rich:tab status="status11" id="tuitions" label="#{msg.tuition}"
				rendered="#{studentBean.studentSelected && studentBean.student.school.id == user.school.id}">
				<a4j:support event="ontabenter" ajaxSingle="true"
					action="#{paymentBean.getStudentPayments}" reRender="studentPanel">
					<f:param name="selectedTab" value="tuitions" />
				</a4j:support>
				<%@include file="/WEB-INF/jsfIncludes/paymentList.jsp"%>
			</rich:tab>

			<rich:tab status="status11" id="exemption" label="#{msg.exemption}"
				rendered="#{studentBean.studentSelected }">
				<a4j:support event="ontabenter" ajaxSingle="true"
			...
			<rich:tab status="status11" id="medicalVisit"
			...
			<rich:tab status="status11" id="curriculum" label="#{msg.curriculum}"
				rendered="#{studentBean.studentSelected && studentBean.student.school.id == user.school.id }">
				<a4j:support event="ontabenter" ajaxSingle="true"
				...
				<rich:tabPanel switchType="ajax" width="100%" id="curriculumPanel"
					...
					<rich:tab id="curriculumProgressPage"
						...
						<a4j:support event="ontabenter" ajaxSingle="true" action="#{curriculumProgressBean.getAll}" 
						reRender="studentPanel">
							<f:param name="selectedTab" value="curriculum" />
			...
			<rich:tab status="status11" id="schoolEvent" label="#{msg.events}"
				rendered="#{studentBean.studentSelected && studentBean.student.school.id == user.school.id }">
				<%@include file="/WEB-INF/jsfIncludes/schoolEventList.jsp"%>
			</rich:tab>

			<rich:tab status="status11" id="timeTablePanelTabs"
				label="#{msg.timeTable}"
				rendered="#{studentBean.studentSelected && studentBean.student.school.id == user.school.id }">
				<rich:tabPanel switchType="ajax" width="100%" id="timeTablePanel"
					immediate="true">
					<rich:tab status="status11" id="viewTimeTable"
						label="#{msg.timeTable}"
						rendered="#{studentBean.studentSelected && studentBean.student.school.id == user.school.id }">
						<%@include file="/WEB-INF/jsfIncludes/timeTable.jsp"%>
					</rich:tab>

					<rich:tab status="status11" id="viewExamTimeTable"
						label="#{msg.exams}"
						rendered="#{studentBean.studentSelected && studentBean.student.school.id == user.school.id }">
			...
			<rich:tab status="status11" id="studentProfile" label="#{msg.profile}"
				rendered="#{studentBean.studentSelected && studentBean.student.school.id == user.school.id }">
				<a4j:support event="ontabenter" ajaxSingle="true"
			...
			<rich:tab status="status11" id="demandes" label="#{msg.request}"
				rendered="#{studentBean.studentSelected && studentBean.student.school.id == user.school.id }">
				<a4j:support event="ontabenter" ajaxSingle="true"
