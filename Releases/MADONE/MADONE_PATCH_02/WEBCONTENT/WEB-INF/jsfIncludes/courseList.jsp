	...
	<a4j:region>
		<a4j:outputPanel id="courses">
			<rich:dataTable rows="5" value="#{courseBean.courses}" var="course"
				...
				<rich:column filterBy="#{course.school.name}" filterEvent="onkeyup"
					sortBy="#{course.school.name}" rendered="#{link != 'school'}">
					<f:facet name="header">
				...
				<rich:column filterBy="#{course.levelClass.name}"
					filterEvent="onkeyup" sortBy="#{course.levelClass.name}" rendered="#{link != 'school'}">
					<f:facet name="header">
				...
				<rich:column filterBy="#{course.teacher.lastName}"
					rendered="#{link=='course' || link == 'school' }" filterEvent="onkeyup"
					sortBy="#{course.teacher.lastName}">
					<f:facet name="header">
						<h:outputText value="#{msg.teacher}" />
					</f:facet>
					<h:outputText
						value="#{course.teacher.lastName} #{course.teacher.firstName} #{course.teacher.nickName}" />
				</rich:column>
				...
				<rich:column filterBy="#{course.courseGroupDesc}"
					filterEvent="onkeyup" sortBy="#{course.courseGroupDesc}">
					<f:facet name="header">
						<h:outputText value="#{msg.courseGroup}" />
					</f:facet>
					<h:outputText value="#{course.courseGroupDesc}" />
				</rich:column>
				...
				<rich:column rendered="#{link != 'school'}">
					...
					<a4j:commandLink ajaxSingle="true" id="modify"
						...
						rendered="#{course.school.id == user.school.id }"
					...
					<a4j:commandLink ajaxSingle="true" id="deleteC"
						...
						rendered="#{course.school.id == user.school.id && link=='course'}"
					...
					<a4j:commandLink ajaxSingle="true" id="deleteT"
						...
						rendered="#{course.school.id == user.school.id && link=='teacher'}"
						reRender="gestionCourse, courses, courseResultPanel"
						oncomplete="#{rich:component('courseResultPanel')}.show();">
						<h:graphicImage value="/images/delete.gif" style="border:0" />
						<f:param name="id" value="#{course.id}" />
