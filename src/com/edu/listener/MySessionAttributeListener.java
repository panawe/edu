...
public class MySessionAttributeListener implements HttpSessionAttributeListener{
	...
	public void attributeAdded(HttpSessionBindingEvent event) {
		...
		if(event.getName().equals("testId")){
			...
			Long testId = new Long(((String) event.getSession().getAttribute("testId")).trim());
	...
	public void attributeRemoved(HttpSessionBindingEvent event) {
		...
		//System.out.println("Removed from session: "+event.getName()+"==>"+event.getValue());
	}

	public void attributeReplaced(HttpSessionBindingEvent event) {
		//System.out.println("Replaced in the session: "+event.getName()+"==>"+event.getValue());
		if(event.getName().equals("testId")){
